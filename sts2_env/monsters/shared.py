...
TORCH_HEAD_AMALGAM_TACKLE_1_MOVE = "STRONG_TACKLE_MOVE"
...
def create_deprecated_monster(rng: Rng) -> tuple[Creature, MonsterAI]:
    creature = Creature(max_hp=0, monster_id="DEPRECATED_MONSTER")

    def stub(combat: CombatState) -> None:
        return

    state = MoveState("STUB", stub, [Intent(IntentType.UNKNOWN)], follow_up_id="STUB")
    return creature, MonsterAI({"STUB": state}, "STUB")
